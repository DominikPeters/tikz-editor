...
export type DesktopLocalAssetReadResult =
  | {
      status: "ok";
      path: string;
      bytesBase64: string;
...
export type DesktopBridge = {
  ...
  compileTikz: (latexDocument: string, options?: { sourceDirectory?: string | null }) => Promise<string>;
...
export function createDefaultBridge(): DesktopBridge {
  ...
  return {
    ...
    readLocalAsset: async (path) => await invoke<DesktopLocalAssetReadResult>("desktop_read_local_asset", { path }),
    ...
    compileTikz: async (latexDocument, options) => await invoke<string>("desktop_compile_tikz", {
      latexDocument,
