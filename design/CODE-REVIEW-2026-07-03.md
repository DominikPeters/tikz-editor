...
- Source-backed option writes are surgical across the generic path and the specialized matrix-cell, tree, resize, arrange, adornment, nested pin-edge, and (as of 2026-07-11) node-positioning writers. Unrelated comments, whitespace, duplicate-entry trivia, and comma style are preserved; normalized serialization remains only for newly synthesized or isolated fragments. Tree-child span resolution still carries an `indexOf` fallback behind exact-slice checks (`property-target.ts`), so Note A's "never re-find text" rule is mitigated there rather than fully enforced.
...
- Tooling now shares a base TypeScript configuration, typechecks desktop, guards generated grammar drift, runs lint/typecheck/dead-code checks in CI, and removes the reviewed workspace debris and duplicate build work. The dead-code guard is a count ratchet (`knip --max-issues`), not an exact per-symbol baseline: lower the count when findings are removed, and note that offsetting additions/removals within the cap pass silently.
...
**Post-review fixes (2026-07-11):** an adversarial review of the 2026-07-10 branch surfaced and fixed: (1) a completion regression from the AST-only symbol rewrite — parser recovery swallows statements after an unterminated brace group, so raw-source scans were restored as a supplement (`completion/index.ts`); (2) standalone-LaTeX and SVG platform export failures bypassing the status-bar error channel (`export-commands.ts`); (3) the node-positioning writers still doing whole-list rejoin (migrated to the preserving rewrite path, with placement entries inserted before an explicit `anchor=`); (4) a pre-existing incremental-replay defect where selectively reused fragments shifted style-chain provenance spans belonging to other statements (`incremental.ts`, metadata-only, wrong inspector jump-to-source during drags); (5) option-list trivia edges — emptying a list with a comment, dangling multiline commas, double separators after trailing commas. Tooling: `lint:ci` now includes `lint:test`, and `apps/desktop/e2e` is typechecked.
